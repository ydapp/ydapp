...
(function($, window) {
	...
	var View = $.Class.extend({
		...
		_tap: function(event) {
			...
			for (; target && target !== document; target = target.parentNode) {
				if (target.tagName === 'A' && target.hash) {
					var hashParams = target.hash.split('/');
					var page = document.getElementById(hashParams[0].replace('#', ''));
		...
		go: function(hashParams) {
			if (this.isInTransition) {
				return;
			}
			var pageSelector = hashParams[0];
			var nextPage = document.querySelector(pageSelector);
			nextPage.hashParams = hashParams;
			if (nextPage) {
